...
function initBudget(
  { budget, tripEnd, tripStart, budgetName, currency },
  userId
) {
...
export function getBudget(budgetId) {
  return db
    ...
    .catch(err =>
      console.error(
        err,
        `something went wrong getting budget with id ${budgetId}`
      )
    )
}
...
export function getPurchases(userId, budgetId) {
  ...
  if (!budgetId || !userId) {
    ...
    return purchaseRef
      ...
      .then(() => purchases)
      .catch(err =>
        console.error(err, 'something went wrong getting purchases')
      )
  }
}
